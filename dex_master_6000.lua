...
useBandageReadyMs = (os.clock() + 1) * 1000
function UseBandage()
...
pulseStatsReadyMs = os.clock() * 1000
while not Player.IsDead and not Player.IsHidden do

    if os.clock() * 1000 > pulseStatsReadyMs then
        Messages.Print("HP: " .. Player.Hits .. " / " .. Player.HitsMax)
        pulseStatsReadyMs = (os.clock() + 2) * 1000
    end
