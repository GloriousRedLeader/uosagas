...
local ACTION_DELAY = 800 -- Server action delay in ms. Adjust if you rubberband.
...
Messages.Print("Starting Gold Consolidation...", Colors.Info)
...
if currentFeetAmount >= MAX_STACK then
    ...
    if movedAny then
        Messages.Print("Finished! Total gold at feet: " .. tostring(currentFeetAmount), Colors.Confirm)
    else
        Messages.Print("No additional nearby gold to consolidate.", Colors.Info)
    end
end